...
const PROTOCOLS = ['DLA', 'PVC', 'SCOPE', 'EAVP', 'PRICE', 'VECTOR'];

interface TerminalLoaderProps {
  isMultiRun?: boolean;
}

const TerminalLoader: React.FC<TerminalLoaderProps> = ({ isMultiRun = false }) => {
  const [logs, setLogs] = useState<string[]>([]);
  const [protocolStatus, setProtocolStatus] = useState<Record<string, 'pending' | 'running' | 'complete'>>(
    Object.fromEntries(PROTOCOLS.map(p => [p, 'pending']))
  );
    ...
    if (isMultiRun) {
      ...
      const runProtocol = () => {
        if (currentIndex < PROTOCOLS.length) {
          const currentProtocol = PROTOCOLS[currentIndex];
